...
@app.command()
def parse(
    doc_id: Optional[str] = typer.Option(None, "--doc-id", help="특정 문서만 파싱"),
    config: Optional[str] = typer.Option(None, "--config", "-c"),
) -> None:
    """수집된 문서를 파싱합니다."""
    cfg = DoltConfig.load(config)
    ...
    for doc in docs:
        try:
            ...
            parsed += 1
            console.print(f"  [green]✓[/green] {doc.file_name} ({structured.total_pages}p, {len(structured.sections)} sections)")
        except Exception as e:
...
def chunk(
    ...
    for doc in docs:
        ...
        if not parsed:
            console.print(f"  [yellow]⚠[/yellow] {doc.file_name}: 파싱 데이터 없음 (dolt parse 먼저 실행)")
            continue
...
@app.command()
def embed(
    provider: EmbeddingProvider = typer.Option(EmbeddingProvider.OPENAI, "--provider", help="openai/cohere/local"),
    model: Optional[str] = typer.Option(None, "--model", help="임베딩 모델"),
...
def export_cmd(
    target: ExportTarget = typer.Option(ExportTarget.JSON, "--target", help="qdrant/pinecone/json/postgres"),
    ...
    console.print(f"\n[green]Export 완료:[/green] {result.success}/{result.total} → {result.destination}")
...
def run(
    ...
    for stage_name in ["ingest", "parse", "chunk", "enrich", "embed", "export"]:
        ...
        if stage:
            status_style = "green" if stage.status == "success" else "yellow" if stage.status == "partial" else "red"
            table.add_row(
                stage_name,
                str(stage.count),
...
def clean(
    ...
    doc_id: Optional[str] = typer.Option(None, "--doc-id", help="특정 문서 데이터 삭제"),
    config: Optional[str] = typer.Option(None, "--config", "-c"),
) -> None:
    """로컬 저장소 데이터를 삭제합니다."""
    import shutil
