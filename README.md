...
You can run the entire pipeline and inspect results in a browser instead of the CLI.
...
- **Dashboard** — Overview of documents, chunks, and embeddings
- **Ingest → Parse → Chunk → Embed → Export** — Run each stage individually
- **Run Pipeline** — Execute the full pipeline at once
- **Plugins** — View installed plugins, enable/disable metadata plugins
